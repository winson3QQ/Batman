...
| `ots.manifest` | generated by `scripts/profile-to-manifest.py ots` — the container list `/usr/bin/payload-run` executes |
| `run.sh` | thin shim (#167 Increment 3): `exec payload-run opentakserver` (bespoke logic is in git history) |
| `batman-payload-opentakserver.init` | the **generic procd guardian** (#156/#167) sourcing `/usr/lib/batman/payload-guardian.sh` — re-brings up the hardened stack + drift-alarms every boot (anti-bypass) |

**Node prep (one-time):** `uci set dockerd.globals.iptables=0; uci commit dockerd; service dockerd restart` (design B single-table — see `docs/design/ots-networking.md`); load the three arm64 images; copy this dir to `/opt/batdata/apps/opentakserver/`.

**Bring up (generic path, #167):** `payload-run opentakserver` — reads `ots.manifest`, applies `ots.fw4.uci`, brings the 6-container stack up hardened on `br-ots`, health-gated. Install the guardian: `cp batman-payload-opentakserver.init /etc/init.d/ && service batman-payload-opentakserver enable && service batman-payload-opentakserver start`. See `docs/design/167-payload-manager.md`.
