...
# --- #148: modern cryptography APIs (pyOpenSSL 24+ removed CRL/Revoked/PKCS12/X509Extension).
# FTS's cert/CRL/p12 generation is ported to `cryptography` so we can run maintained
# pyOpenSSL>=26 + cryptography>=42 (clears CVE-2023-50782 / CVE-2024-0727 / CVE-2026-27459).
# Behaviour and on-disk formats are preserved (PEM, ca.pem CRL append, revoked_file JSON, p12).
import datetime as _dt148
from cryptography import x509
from cryptography.x509.oid import NameOID
from cryptography.hazmat.primitives import hashes as _hashes148, serialization as _ser148
from cryptography.hazmat.primitives.asymmetric import rsa as _rsa148
from cryptography.hazmat.primitives.serialization import pkcs12 as _pkcs12_148

_CRL_NEXT_UPDATE_DAYS = 365  # #148: pyOpenSSL export() defaulted this; set explicit. Long-lived (tactical CA).

def _new_rsa_key_148():
    return _rsa148.generate_private_key(public_exponent=65537, key_size=2048)

def _key_to_pem_148(key):
    # Traditional OpenSSL PEM (unencrypted) — matches what load_pem_private_key / OpenSSL reads back.
    return key.private_bytes(_ser148.Encoding.PEM, _ser148.PrivateFormat.TraditionalOpenSSL, _ser148.NoEncryption())
...
def revoke_certificate(username, revoked_file=None, ca_pem = config.CA, ca_key = config.CAkey, crl_file = config.CRLFile, user_cert_dir=config.certsPath, crl_path=config.CRLFile):
    ...
    data = {}
    # #148: load CA cert/key + build the CRL with `cryptography` (pyOpenSSL CRL API removed in 24+).
    ca_cert = x509.load_pem_x509_certificate(open(ca_pem, "rb").read())
    ca_priv = _ser148.load_pem_private_key(open(ca_key, "rb").read(), password=None)

    now = datetime.now(timezone.utc)
    builder = (x509.CertificateRevocationListBuilder()
               .issuer_name(ca_cert.subject)
               .last_update(now)
               .next_update(now + _dt148.timedelta(days=_CRL_NEXT_UPDATE_DAYS)))

    if crl_path and os.path.exists(crl_path):
        # seed the new CRL from the existing one (preserve previously-revoked entries)
        existing_crl = x509.load_pem_x509_crl(open(crl_path, "rb").read())
        for _r in existing_crl:
            builder = builder.add_revoked_certificate(_r)
    else:
    ...
    for cert in os.listdir(user_cert_dir):
        if cert.lower() == f"{username.lower()}.pem":
            with open(config.certsPath+'/'+cert, 'rb') as _fh:
                revoked_cert = x509.load_pem_x509_certificate(_fh.read())
            data[str(revoked_cert.serial_number)] = username
            break

    for key in data:
        revoked = (x509.RevokedCertificateBuilder()
                   .serial_number(int(key))       # decimal serial (cryptography wants an int)
                   .revocation_date(now)
                   .build())
        builder = builder.add_revoked_certificate(revoked)

    crl = builder.sign(private_key=ca_priv, algorithm=_hashes148.SHA256())
    crl_pem = crl.public_bytes(_ser148.Encoding.PEM)
...
class AtakOfTheCerts:
    def __init__(self, pwd: str = config.password) -> None:
        ...
        """
        self.key = None  # #148: holds a `cryptography` RSA private key after _generate_key
        self.CERTPWD = pwd
    ...
    def generate_ca(self, expiry_time_secs: int = 31536000) -> None:
        ...
        # #148: build the self-signed CA cert with `cryptography` (X509Extension removed in pyOpenSSL 26).
        ca_key = _new_rsa_key_148()
        ...
        now = _dt148.datetime.now(_dt148.timezone.utc)
        cert = (x509.CertificateBuilder()
                ...
                .not_valid_after(now + _dt148.timedelta(seconds=expiry_time_secs))
                .add_extension(x509.BasicConstraints(ca=True, path_length=None), critical=False)
                .add_extension(x509.KeyUsage(digital_signature=False, content_commitment=False,
                                             key_encipherment=False, data_encipherment=False,
                                             key_agreement=False, key_cert_sign=True, crl_sign=True,
                                             encipher_only=False, decipher_only=False), critical=False)
                .sign(ca_key, _hashes148.SHA256()))

        f = open(self.cakeypath, "wb")
        f.write(_key_to_pem_148(ca_key))
        f.close()

        f = open(self.capempath, "wb")
        f.write(cert.public_bytes(_ser148.Encoding.PEM))
        f.close()
        ...
        crl = (x509.CertificateRevocationListBuilder()
               ...
               .last_update(now)
               .next_update(now + _dt148.timedelta(days=_CRL_NEXT_UPDATE_DAYS))
               .sign(private_key=ca_key, algorithm=_hashes148.SHA256()))
        crl_pem = crl.public_bytes(_ser148.Encoding.PEM)
    ...
    def _generate_key(self, keypath: str) -> None:
        ...
        if os.path.exists(keypath):
            ...
            self.key = _new_rsa_key_148()          # #148: cryptography RSA key (was crypto.PKey)
            f = open(keypath, "wb")
            f.write(_key_to_pem_148(self.key))
            f.close()
    ...
    def _generate_certificate(self, common_name: str, p12path: str, pempath: str = config.pemDir,
        ...
        if not os.path.exists(pempath):
            # #148: cryptography port (crypto.X509 build kept faithful; PKCS12 API removed in pyOpenSSL 24+)
            ca_key = _ser148.load_pem_private_key(open(self.cakeypath, "rb").read(), password=None)
            ...
            cert = (x509.CertificateBuilder()
                    ...
                    .not_valid_after(now + _dt148.timedelta(seconds=expiry_time_secs))
                    .sign(ca_key, _hashes148.SHA256()))
            # NOTE(#148): BestAvailableEncryption is AES-based PKCS12; pyOpenSSL's legacy export used
            # older PBE. Modern ATAK/Java read AES p12, but VERIFY client .p12 import in the field test.
            p12data = _pkcs12_148.serialize_key_and_certificates(
                name=None,
                key=self.key,
                cert=cert,
                cas=[ca_pem],
                encryption_algorithm=_ser148.BestAvailableEncryption(bytes(self.CERTPWD, encoding='UTF-8')),
            )
