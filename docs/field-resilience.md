...
4b. **Field status without hardware (the phone is always there):** the node serves its own incident report at `http://<hostname>.lan/cgi-bin/status` (HTML for a phone on the onboarding AP, `?json` for machines) and `/cgi-bin/bundle` (the attachment). This is the minimum a person in the field needs to *report* a node — the two Pi LEDs are not visible through the V3 enclosure until a light pipe / panel LED is added (#128).
