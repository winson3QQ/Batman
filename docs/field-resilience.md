...
4. **Join watchdog (N1):** after boot, if no plink within T1 (default 5 min): record a *join diagnosis* line — radio state, channel/BW, mesh id, key-set?, peers heard (scan), batman neighbours, bootstrap/reserved address — to `log/join.log` (one line per attempt, bounded) and show a distinct LED pattern; keep retrying without reboot; after T2 (default 60 min) one controlled reboot, then never again automatically (a loop here is worse than a static failure that a human can read). Never touch the radio config on its own.
