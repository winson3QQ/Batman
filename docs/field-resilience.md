...
| Pi 3A+ / Pi 3 family (no EEPROM bootloader) — *claim inherited from the Zero 2 W; `tryboot.txt` on Pi 3 is unverified, #203 risk 5* | only the file-level `tryboot.txt` switch on the single boot partition → A/B of the rootfs and of the kernel files (`os_prefix=`), never of the boot partition itself. Accepted for the relay/expendable role (productization.md). |
