...
1. 用上週**自己開的 5 條規格**，畫黑箱（外面看得到什麼）與白箱（裡面有什麼元件）。
2. 列**介面清單**：哪兩個元件碰在一起？至少 6 條。格式：「A ↔ B：要對得上什麼」。
3. 挑一條規格，畫「規格 → 元件」連線圖，數一數牽到幾樣、幾個學門。

**AI 進場（扮第二位系統工程師，只做一件事）**
「你是這個案子的第二位系統工程師。以下是我的元件清單和介面清單。只做一件事：指出我漏掉的介面，每個用一句話說為什麼它會出問題。不要幫我畫圖，不要補其他東西。」
學生逐條判定「真的漏了」或「不算」，理由寫進筆記。
...
- 系統／元件／介面，我用腳踏車以外的例子各一句：___
- 本專案三個介面故障，我最有感的是 ___，因為 ___。
- 我的規格「___」牽到 ___ 個元件、___ 個學門。
- AI 說我漏了 ___，我判定 ___。
