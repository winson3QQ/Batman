...
- **stop()** (runs on `reboot`/`halt`/sysupgrade — any clean shutdown): dumps the whole syslog
  ...
  with uptime + **what triggered it** (a daemon's own reason left in `/tmp/shutdown.reason` —
  `low-battery Vbat=…` from batpower #122, `joinwatch: not joined … automatic reboot (once)`
  #127; sysupgrade; openmanetd's one-time addressing reboot — "Rebooting system to apply new
  network settings", expected once after provisioning; or the reboot command + the last SSH
  login source). Timestamps are best-effort (no RTC/NTP): `boot_id` and `uptime=` are the
  reliable keys.
