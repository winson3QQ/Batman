...
- **stop()** (runs on `reboot`/`halt`/sysupgrade — any clean shutdown): dumps the whole syslog
  ...
  with uptime + **what triggered it** (sysupgrade; openmanetd's one-time addressing reboot —
  "Rebooting system to apply new network settings", expected once after provisioning; or the
  reboot command + the last SSH login source).
